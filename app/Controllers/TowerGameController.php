...
class TowerGameController extends ResourceController
{
    ...
    public function returnInitialState( $difficulty ){
        $rodA = [];
        for ($i=0; $i < $difficulty ; $i++) { 
            $j = $i+1;
            array_push( $rodA, $j );
        }
        return array_reverse( $rodA );
    }
    ...
    {   
        ...
        //     'C' => []
        // ]);
        $rodA = $this->returnInitialState( $difficulty );
        $initialState = json_encode([
            'A' => $rodA, // 7 is the largest, 1 is the smallest
            'B' => [],
            'C' => []
        ]);
